...
class Polynom
{
  public:
   ...
   // polynomial and RHS are unchanged
   ...
   // returned; this polynomial and RHS are unchanged
   ...
   // Postcondition: If precondition is satisfied the coefficient of x^k is
   ...
   //void insert(ostream& oStr) const;
   
   // Stream extraction.
   //  
   // Postcondition: Read the coefficients of this polynomial from stream iStr 
   // Example: if the user entered the following
   //              -1 1 0 0 2 14 0
   //           at the keyboard, this Polynom object will replace data member
   //           P's current content with [ -1 1 0 0 2 14 0] 
   
   istream& extract(istream& iStr);
   //void extract(istream& iStr);
  private:
   vector<int> P;     // coefficients of the polynomial; if P is nonempty and
   // P.size() is n then P[i] is the coefficient of x raised
   // to power (n-i-1)

   ostream& printTerm(char sign, int pwr, int coeff, ostream& sout) const;
     
};

// overloaded stream to write the polynomial rhs onto stream oStr 
